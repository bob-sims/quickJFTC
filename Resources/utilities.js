exports.escapeString = function(_string) {
    return typeof _string != "string" ? "\"" + _string + "\"" : "\"" + _string.replace(/"/g, "'") + "\"";
};
...
exports.htmlDecode = function(_string) {
    var tmp_str = _string.toString(), hash_map = exports.htmlTranslationTable(), results = tmp_str.match(/&#\d*;/ig);
    if (results) for (var i = 0, x = results.length; i < x; i++) {
...
exports.toDateAbsolute = function(_date) {
    ...
    var dateMinutes = date.getMinutes() < 10 ? "0" + date.getMinutes() : date.getMinutes(), datePeriod = date.getHours() > 12 ? "PM" : "AM", months = [ "January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December" ];
...
exports.toDateRelative = function(_date) {
    var date = new Date;
    date.setTime(_date);
    var now = new Date, days = [ "Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday" ], dateMonth = date.getMonth(), dateDate = date.getDate(), dateDay = days[date.getDay()], dateYear = date.getFullYear(), dateHour = date.getHours() > 11 ? date.getHours() - 12 : date.getHours();
    dateHour = dateHour == 0 ? 12 : dateHour;
    var dateMinutes = date.getMinutes() < 10 ? "0" + date.getMinutes() : date.getMinutes(), datePeriod = date.getHours() > 12 ? "PM" : "AM", nowMonth = now.getMonth(), nowDate = now.getDate(), nowYear = now.getFullYear();
    return dateYear == nowYear && dateMonth == nowMonth ? dateDate == nowDate ? "Today " + dateHour + ":" + dateMinutes + datePeriod : dateDate >= nowDate - 1 ? "Yesterday " + dateHour + ":" + dateMinutes + datePeriod : dateDate >= nowDate - 6 ? dateDay + " " + dateHour + ":" + dateMinutes + datePeriod : exports.toDateAbsolute(_date) : exports.toDateAbsolute(_date);
};
...
exports.stripTags = function(str, allowed_tags) {
    var key = "", allowed = !1, matches = [], allowed_array = [], allowed_tag = "", i = 0, k = "", html = "", replacer = function(search, replace, str) {
        return str.split(search).join(replace);
    };
    allowed_tags && (allowed_array = allowed_tags.match(/([a-zA-Z0-9]+)/gi));
    str += "";
    matches = str.match(/(<\/?[\S][^>]*>)/gi);
    for (key in matches) if (key) {
        html = matches[key].toString();
        allowed = !1;
        for (k in allowed_array) if (k) {
            allowed_tag = allowed_array[k];
            i = -1;
            i != 0 && (i = html.toLowerCase().indexOf("<" + allowed_tag + ">"));
            i != 0 && (i = html.toLowerCase().indexOf("<" + allowed_tag + " "));
            i != 0 && (i = html.toLowerCase().indexOf("</" + allowed_tag));
            if (i == 0) {
                allowed = !0;
